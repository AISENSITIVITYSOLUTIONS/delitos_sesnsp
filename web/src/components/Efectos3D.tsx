...
const targets='.impacto-kpis article, .hallazgo, .resumen-grid>article';
export default function Efectos3D(){
 ...
 useEffect(()=>{
  ...
  const seen=new WeakSet<Element>();const discover=()=>document.querySelectorAll('.impacto-hero, .encabezado__sello').forEach(e=>{if(!seen.has(e)){seen.add(e);observer.observe(e);}});discover();
