...
const pct=(n:number|null)=>n==null?'No comparable':`${n>0?'+':''}${fmt(n)} %`;
const enfoques=[
 ...
 ['3 · Composición','¿Qué delitos mejoran?','Observa cada categoría por separado. Un descenso en robo no permite concluir que también descendieron homicidio, feminicidio o extorsión. No se calcula un total de esta selección editorial.'],
...
export default function AltoImpacto({nm,nr,pob}:{nm:SeriesNacionales;nr:SeriesNacionales|null;pob:PoblacionAnual}){
 ...
 const ids=PRIORITARIOS.filter(k=>sn.series[k]);const delito=ids.includes(id)?id:ids[0];
 ...
 return <section className="seccion impacto" id="alto-impacto" aria-labelledby="t-impacto">
  ...
  <p className="seccion__desc">Selección analítica de NI, no una canasta oficial certificada. Los robos disponibles incluyen sus modalidades agregadas: no equivalen a «robo con violencia». Se muestran delitos, no víctimas ni carpetas. No se unen automáticamente las metodologías 2015–2025 y 2026.</p>
  <div className="impacto-controles">
   ...
   <label>Delito<select className="control" value={delito} onChange={e=>setId(e.target.value)}>{ids.map(k=><option value={k} key={k}>{sn.jerarquia[k].subtipo}</option>)}</select></label>
  ...
  <div className="impacto-kpis" aria-live="polite">
   <article><span>{sn.jerarquia[delito].subtipo} · {actual}</span><strong>{fmt(a)}</strong><small>{unidad}</small></article>
   <article><span>Referencia · {base}</span><strong>{fmt(b)}</strong><small>Misma unidad y mismo instrumento</small></article>
   <article><span>Cambio frente a la referencia</span><strong>{pct(dif)}</strong><small>{actual===base?'Se está comparando el mismo mes.':actual.slice(5)!==base.slice(5)?'Meses distintos: puede intervenir la estacionalidad.':'Mismo mes del calendario; no implica causalidad.'}</small></article>
  </div>
  ...
  <div className="tarjeta impacto-panel"><h3>{enfoques[tab][1]}</h3><p>{enfoques[tab][2]}</p>
   {tab===0&&<><Grafica ariaLabel={`Serie mensual de ${sn.jerarquia[delito].subtipo}`} opcion={{xAxis:{type:'category',data:meses,axisLabel:{color:'#8fa9bf'},axisLine:{lineStyle:{color:'#446078'}}},yAxis:{type:'value',min:0,axisLabel:{color:'#8fa9bf'},splitLine:{lineStyle:{color:'#345063'}}},series:[{type:'line',data:meses.map(m=>valor(delito,m)),connectNulls:false,symbol:'none',lineStyle:{color:'#169bff',width:3},areaStyle:{color:'#169bff',opacity:0.12}}]}}/><details><summary>Ver tabla accesible</summary><div className="impacto-tabla"><table><thead><tr><th>Mes</th><th>Registros</th><th>{unidad}</th></tr></thead><tbody>{meses.map(m=><tr key={m}><td>{m}</td><td>{fmt(valorMes(sn,delito,m))}</td><td>{fmt(valor(delito,m))}</td></tr>)}</tbody></table></div></details><button className="boton" onClick={()=>descargarCSV('ni-alto-impacto.csv',['mes','registros',unidad],filas,{fuente:sn.meta.fuente,url:sn.meta.url_fuente,corte:sn.meta.corte,version:String(sn.meta.version),delito,metrica:unidad,instrumento:sn.metodologia,poblacion:pob.meta.fuente,version_poblacion:String(pob.meta.version)})}>Descargar serie CSV</button></>}
   {tab===1&&<div className="impacto-tabla"><table><thead><tr><th>Base, mismo mes</th><th>Valor base</th><th>Valor {actual}</th><th>Variación</th></tr></thead><tbody>{meses.filter(m=>m<actual&&m.slice(5)===actual.slice(5)).map(m=><tr key={m}><td>{m}</td><td>{fmt(valor(delito,m))}</td><td>{fmt(a)}</td><td>{pct(cambio(a,valor(delito,m)))}</td></tr>)}</tbody></table>{!meses.some(m=>m<actual&&m.slice(5)===actual.slice(5))&&<p>No hay un año anterior comparable dentro de este instrumento. No se empalma con 2025 sin correspondencia validada.</p>}</div>}
   {tab===2&&<div className="impacto-tabla"><table><thead><tr><th>Categoría editorial NI</th><th>{base}</th><th>{actual}</th><th>Cambio</th></tr></thead><tbody>{ids.map(k=><tr key={k}><td>{sn.jerarquia[k].subtipo}</td><td>{fmt(valor(k,base))}</td><td>{fmt(valor(k,actual))}</td><td>{pct(cambio(valor(k,actual),valor(k,base)))}</td></tr>)}</tbody></table></div>}
   {tab===3&&<div className="impacto-protocolo"><h4>Protocolo de verificación</h4><ol><li>Registrar cita, emisor, fecha y enlace al documento original.</li><li>Precisar víctimas o delitos; fuero, ámbito y categorías incluidas.</li><li>Reproducir ambos valores con la misma versión de datos.</li><li>Contrastar meses equivalentes, promedio diario, tasas y ventanas de 12 meses cuando sean comparables.</li><li>Clasificar: reproducible, reproducible con matices, no comparable o evidencia insuficiente.</li></ol><p><strong>Estado: sin declaraciones verificadas cargadas.</strong> Los registros de este sitio no permiten certificar anuncios referidos a víctimas.</p></div>}
   {tab===4&&<div className="impacto-protocolo"><ul><li><a href="https://www.inegi.org.mx/programas/mortalidad/">INEGI · Defunciones registradas</a>: confrontar homicidios por año de ocurrencia y de registro, con su rezago y cobertura.</li><li><a href="https://www.inegi.org.mx/contenidos/programas/envipe/2025/doc/envipe2025_presentacion_nacional.pdf">ENVIPE 2025 · documento verificado</a>: victimización de 2024; percepción de marzo–abril de 2025. No mide el mismo periodo ni universo que los registros mensuales.</li><li><a href="https://gabinetedeseguridad.gob.mx/">Gabinete de Seguridad</a>: localizar las declaraciones originales y sus anexos antes de evaluarlas.</li></ul><p>Fuentes externas todavía no integradas numéricamente. No se aplica un factor universal de cifra negra ni se atribuye causalidad a una administración con una comparación antes/después.</p></div>}
  </div>
